...
export class CourseController {
  ...
  async createCourse(req: Request, res: Response) {
    try {
      const { title, description, telegramLink, startAt, duration } = req.body;
      // Validate required fields
      if (!title || !description || !telegramLink || !startAt || !duration) {
        return res.status(400).json({
          message:
            "Title, description, telegramLink, startAt, and duration are required",
        });
      }
  ...
  async updateCourse(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const { title, description, telegramLink, startAt, duration, isActive } =
        req.body;
  ...
  async registerForCourse(req: Request, res: Response) {
    try {
      ...
      const userId = (req as any).user?._id; // Optional user ID if logged in
      ...
      const existingRegistration = course.registrations.find(
        (reg) => reg.email === email
      );
      if (existingRegistration) {
        return res.status(400).json({ 
          message: "This email is already registered for this course" 
        });
      }
      ...
      return res
        .status(500)
        .json({ message: "Error registering for course", error });
    }
  }
  ...
  async getCourseRegistrations(req: Request, res: Response) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Error fetching course registrations", error });
    }
  }
  ...
  async getActiveCourses(req: Request, res: Response) {
    try {
      const activeCourses = await Course.find({ isActive: true }).populate(
        "registrations.userId"
      );
      ...
      return res
        .status(500)
        .json({ message: "Error fetching active courses", error });
    }
  }
}
